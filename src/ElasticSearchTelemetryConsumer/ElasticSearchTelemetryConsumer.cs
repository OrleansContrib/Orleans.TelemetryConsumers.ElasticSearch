		...
		public IElasticLowLevelClient GetClient(Uri esurl)
		{
			...
			else
			{
				...
				var cc = new ConnectionConfiguration(singleNode,
						connectionSettings => new ElasticsearchJsonNetSerializer(OrleansJsonSerializer.    .GetDefaultSerializerSettings() .GetDefaultSerializerSettings() .GetDefaultSerializerSettings().Converters))
